...
CAF::CAF(const std::string &filename,
         const std::string &sandFilename,
         const std::string &rw_fhicl_filename,
         bool makeFlatCAF,
         bool storeGENIE)
...
//  }
  ...
  // new meta branches 
  cafPOT->Branch("selection_type",&selection_type,"selection_type/I");
  cafPOT->Branch("interaction_type",&interaction_type,"interaction_type/I");
  cafPOT->Branch("ecal_reco_type",&ecal_reco_type,"ecal_reco_type/I");
  cafPOT->Branch("grain_reco_type",&grain_reco_type,"grain_reco_type/I");
  cafPOT->Branch("tracker_reco_type",&tracker_reco_type,"tracker_reco_type/I");


  const std::map<std::string, SelectionType> selectionMap = {
    {"graincontained",   SelectionType::kGRAINcontained},
    {"ecalcontained",    SelectionType::kECalcontained},
    {"trackercontained", SelectionType::kTrackercontained}
  };

  const std::map<std::string, InteractionType> interactionMap = {
    {"QE", InteractionType::kQE}
  };

  const std::map<std::string, RecoType> recoMap = {
    {"RealReco", RecoType::kRealReco},
    {"Smearing", RecoType::kSmearing},
    {"None",     RecoType::kNone}
  };

  
  std::string name = sandFilename.substr(sandFilename.find_last_of("/")+1);

  name = std::regex_replace(name,std::regex("\\.root"),"");
  ...
  if(tokens.size() < 7){
    std::cerr << "ERROR: filename format not recognized: "
              << sandFilename << std::endl;
    abort();
  }

  try{

    selection_type    = selectionMap.at(tokens[2]);
    interaction_type  = interactionMap.at(tokens[3]);
    ecal_reco_type    = recoMap.at(tokens[4]);
    grain_reco_type   = recoMap.at(tokens[5]);
    tracker_reco_type = recoMap.at(tokens[6]);

  }
  catch(const std::out_of_range &){

    std::cerr << "ERROR parsing SANDReco filename: "
              << sandFilename << std::endl;
...
int CAF::StoreGENIEEvent(const genie::NtpMCEventRecord *evtIn){
  ...
  if(evtIn != mcrec){
    ...
    if(!warned){
      ...
      cafmaker::LOG_S("CAF::StoreGENIEEvent()")
        << "Repeatedly reassigning GENIE pointer will slow down performance\n";
