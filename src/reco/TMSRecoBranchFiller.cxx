...
namespace cafmaker
{
  TMSRecoBranchFiller::TMSRecoBranchFiller(const std::string &tmsRecoFilename)
  {
    ...
    if (!fTMSRecoFile->IsZombie()) {
      ...
      TMSRecoTree->SetBranchAddress("DirectionX", _DirectionX);
      TMSRecoTree->SetBranchAddress("DirectionZ", _DirectionZ);
      ...
      const cafmaker::Params &par) const
  {
    ...
    for (int i = 0; i < _nLines; ++i) {
      ...
      sr.nd.tms.tracks[i].dir = caf::SRVector3D(_DirectionX[i], -999, _DirectionZ[i]);
      // maybe want to fill this out?
      sr.nd.tms.tracks[i].enddir = caf::SRVector3D(-999, -999, -999);
    }
